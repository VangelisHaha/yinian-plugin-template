...
在**桌面端浮窗**上多一张卡片。浮窗的形态是一列卡片（一念内置的今日时间轴 / 今日待办 / 接下来 / 明天 / 完成进度），你的卡片和它们并排，由用户在设置页的卡片画廊里逐个打开并排序。

**桌面端没有系统小组件面板可用**——macOS 通知中心与 Windows 11 小组件面板都不对第三方应用开放同等能力。所以「桌面端小组件」就是浮窗上的一张卡片。

- **默认关闭，第一次被调到就是授权信号。** 与 `calendarOverlay` 同档，而位置更靠前：浮窗是**常驻置顶窗口**，可能出现在会议投屏、结对编程、录屏里。「装了一个飞书插件」不等于「同意把我的会议标题画在屏幕最上层」。**在收到第一次 `widget.render` 之前，不要去拉用户的个人数据。**
- **`layout` 是四种封闭布局之一**（`list` / `metrics` / `timeline` / `text`），**没有 `custom`、不能给 HTML、不能给 iframe**。留了那个逃生口，所有插件都会走它，四种布局就白定义了；而 iframe 里的 `fetch` 还会绕过你在 manifest 里声明的 `permissions.net`，你的联网面从此不可审计。写错 `layout` 会让插件**装不上**，不会静默退化成 `list`。
- **`description` 是必填的**（`calendarOverlay` 那边可选）：它画在常驻置顶窗口上，用户凭这句话决定要不要启用。
- 超时 **8 秒**，比 `dayMarks.list` 还紧——浮窗按一下热键就要出来。**不要在这里发网络请求**，只读 `dataDir` 里的缓存。`refreshSeconds` 声明的是「宿主多久来问一次」，不是「你多久去拉一次」，两者独立。
- **没数据时正常返回并给 `empty`，不要抛异常。** 抛出去用户看到的是「暂时取不到」，而「今天没有会议了」是个好消息，不该长得像错误。失败时宿主会**保留卡片**、只换内容——所以你也不必担心一次超时让卡片消失。
- **`now` 由宿主下发，不要自己读钟。** 宿主画「现在线」用的是同一个时刻；各读一次的结果是轨道上的现在线与行尾的倒计时差出几秒到几分，而那种不一致看起来就是 bug。
- **`time` 与 `value` 都是已格式化好的字符串**，宿主原样显示、不做算术。`layout: "timeline"` 时另给 `startMinute` / `endMinute`；**给不出分钟数的条目不要丢**，宿主会把它退化成普通行排在后面——一个「时间待定」的会议仍然是今天要开的会。
- **颜色与字体归宿主**：`tone` 是语义档位（同 `calendarOverlay`），`mark` 是三种封闭形状（`none` / `dot` / `bar`）。
- **`action` 只有 `open` 与 `deeplink`**，没有 `write`、没有 `rpc`——小组件是只读投影。`open` **必须 `https://`**（`http://` 也会被拒），宿主会在行上显示域名。协议不符时宿主**只摘掉 action、保留这一行**。
- 上限：`list` / `timeline` 8 行、`metrics` 4 项、`text` 3 行，超出丢弃；**没有 `primary` 的行整条丢弃**（那在界面上是一片留白）。
- **声明 `list` 却顺带返回 `metrics` 会被清掉。** 按 `layout` 给你该给的那一种。
